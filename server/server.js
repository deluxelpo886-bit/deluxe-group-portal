...
try {
  ...
  if (fs.existsSync(seedPath)) {
    ...
    const r = serviceLog.applySeed(seed, 'fleet-asset-list-2026-09-24-r26');
...
try {
  ...
  if (fs.existsSync(hireSeedPath)) {
    ...
    const r = hire.applySeed(hs, 'fleet-hire-2026-09-26d');
...
try {
  ...
  if (fs.existsSync(flagSeedPath)) {
    ...
    const r = dataflags.applySeed(fseed, 'fleet-dataflags-2026-09-16a');
...
try {
  ...
  if (fs.existsSync(spareSeedPath)) {
    ...
    const r = spares.applySeed(sseed, 'fleet-spares-2026-09-14c');
...
try {
  const PRUNE_RM = ['DG-476-crosscheck-2026-09-19', 'hydropower-deliver-2026-09-19', 'delcotech-40kva-ready-2026-09-19', 'DG-430-hire-details-2026-09-21', 'motherson-gatepass-2026-09-25', 'DG-868-arrival-to-yard-2026-09-23', 'DG-537-overdue-breakdown-replace-2026-09-25', 'DG-473-14day-cycle-2026-10-08'];
  let removed = 0;
  reminders.getAll().forEach((r) => {
    if (r && r.seedKey && PRUNE_RM.indexOf(r.seedKey) !== -1) { reminders.remove(r.id); removed += 1; }
  });
  if (removed) console.log('[prune] removed ' + removed + ' superseded reminder(s)');
} catch (e) { console.warn('[prune] reminders skipped:', e && e.message); }
...
try {
  ...
  if (fs.existsSync(rentalSeedPath)) {
    ...
    const r = rentals.applySeed(rs, 'fleet-rentals-2026-09-10a');
